...
D=Path(__file__).parent;data=json.loads(gzip.decompress((D/'outputs.json.gz').read_bytes()));base=json.loads((D.parent/'S1/inputs-valid.json').read_text())['base']['state'];groups={}
def dist(xs):
...
for (role,a),rs in groups.items():
 ...
 out.append({'role':role,'approach':a,'n':len(rs),'metrics':metrics,'startsPerAppearance':sum(r['output']['report']['stats']['starts'] for r in rs)/totalApps if totalApps else None,'injured':sum(bool(r['output']['report']['injury']) for r in rs),'oldRisk':dist([r['oldRiskFormulaForComparison'] for r in rs]),'roleAfter':roleAfter})
