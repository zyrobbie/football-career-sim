...
const D='/tmp/PSU-20260912-coordinator/ui-final-r3',origin='http://127.0.0.1:4289';fs.mkdirSync(D,{recursive:true});
...
const scenarios=['PUSH','STEADY','TEAM_FIRST'].map(a=>({id:a,state:{...structuredClone(base),phase:'HALF_YEAR_PLAN',trainingFocus:null,developmentApproach:null},approach:a}));
for(const group of ['mature','veteran-sharp']){const r=continuous.find(r=>r.group.id===group&&r.approach==='PUSH');scenarios.push({id:group+'-recovery',state:r.final,approach:'PUSH'});}
const results=[];
(async()=>{const browser=await chromium.launch({headless:true});try{for(const [width,height] of [[1280,720],[390,844]])for(const scenario of scenarios){
