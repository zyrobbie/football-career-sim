...
- `crestKey`：稳定资产键；
- `crestSourceUrl`：来源页，不直接写入存档；
- `crestLicenseNote`：授权或使用状态；
- `crestStatus`：`approved`、`review` 或 `fallback`；
- `shortMark`：无可用队徽时的文字回退。

即使项目不商业化，也不默认所有俱乐部队徽可以自由复制。首版可在审核后分批接入，并保证删除某个队徽文件不会影响存档、俱乐部身份或游戏逻辑。
