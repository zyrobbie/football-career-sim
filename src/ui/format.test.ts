...
describe('club level labels', () => {
  it('distinguishes global platform and training levels across leagues', () => {
    ...
    expect(clubLevelLabel(shanghai)).toBe('中国顶级联赛豪门')
    expect(clubLevelLabel(guangxi)).toBe('中国第二级别联赛球队')
    expect(trainingQualityLabel(inter)).toBe('世界顶级训练条件')
    expect(trainingQualityLabel(shanghai)).toBe('国内顶尖训练条件')
  ...
  it('distinguishes second divisions through V1 division levels', () => {
    ...
    expect(clubLevelLabel(burnley)).toBe('五大联赛国家第二级别强队')
    expect(clubLevelLabel(liaoning)).toBe('中国第二级别联赛球队')
  })
})
