...
运行时目录现有 366 家俱乐部：32 家中国虚构俱乐部、334 家海外真实名称俱乐部，覆盖 19 个联赛。它们已进入训练质量、合同和转会候选流程，旧运行时 ID 仍保持兼容；不模拟完整阵容、教练名单、真实赛程或动态升降级。队徽层已接入 12 家样板：4 家中国虚构俱乐部使用原创 SVG；其余俱乐部（包括待审核的真实俱乐部资产）继续使用文字短标回退。
