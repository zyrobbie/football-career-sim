...
运行时目录现有 366 家俱乐部：32 家中国虚构俱乐部、334 家海外真实名称俱乐部，覆盖 19 个联赛。它们已进入训练质量、合同和转会候选流程，旧运行时 ID 仍保持兼容；不模拟完整阵容、教练名单、真实赛程或动态升降级。32 家中国虚构俱乐部均已接入本地原创 SVG 队徽；334 家海外俱乐部继续使用文字短标。`CLUB_CREST_MANIFEST` 另保留 8 条现实俱乐部身份参考记录，但这些记录的 `assetPath` 为 `null`，不会渲染真实队徽；项目未宣称完成 334 家海外原创队徽。
