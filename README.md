...
职业半年新增关键比赛时刻：五位置组共40个场景，三选一并保存结果后结算；近期四次优先不重复，旧选择与报告快照保持。
...
运行时目录现有 366 家俱乐部：32 家中国虚构俱乐部、334 家海外真实名称俱乐部，覆盖 19 个联赛。它们已进入训练质量、合同和转会候选流程，旧运行时 ID 仍保持兼容；不模拟完整阵容、教练名单、真实赛程或动态升降级。32 家中国虚构俱乐部及已验收的海外俱乐部队徽，通过[队徽清单](src/data/clubs/clubCrests.ts)映射到本地原创资产；尚未接入的俱乐部使用文字短标。`CLUB_CREST_MANIFEST` 另保留 5 条 `assetPath: null` 的现实身份参考记录，它们不会渲染真实队徽；项目未宣称完成 334 家海外原创队徽。
